...
// func TestHuffmanService_Compress(t *testing.T) {
...
// 			h := &HuffmanService{}
// 			if got := h.Compress(tt.data); !reflect.DeepEqual(got, tt.want) {
// 				t.Errorf("HuffmanService.Compress() = %v, want %v", got, tt.want)
// 			}
// 		})
// 	}
// }

func TestHuffmanService_Decompress(t *testing.T) {
	tests := []struct {
		name string
		data []byte
	}{
		...
		{
			name: "",
			data: []byte("Сжатие_Хаффмана просто лучшее"),
		},
		{
			name: "",
			data: []byte("Сжатие_Хаффмана просто лучшее, я вам отвечаю. слово даю!!"),
		},
		{
			name: "",
		...
		{
			name: "",
			data: []byte(`Лев Николаевич Толстой
Война и мир
...
XVIII
   ...
   -- Вы к князю Кириллу Владимировичу, моя милая, -- сказал граф из столовой, выходя тоже в переднюю. -- Коли ему лучше, зовите Пьера ко мне обедать. Ведь он у меня бывал, с детьми танцевал. Зовите непременно... Hy, посмотрим, как-то отличится нынче Тарас. Говорят, что у графа Орлова такого обеда не бывало, какой у нас будет.`),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			h := &HuffmanService{}
			compressedData, err := h.Compress(tt.data)
			if err != nil {
				t.Errorf("HuffmanService.Compress() has err = %s", err.Error())
				return
			}
			decompressedData, err := h.Decompress(compressedData)
			if err != nil {
				t.Errorf("HuffmanService.Decompress() has err = %s", err.Error())
				return
			}
			if !reflect.DeepEqual(tt.data, decompressedData) {
				t.Errorf("HuffmanService.Compress() = %s, want %s", decompressedData, tt.data)
			}
		})
	}
}
...
func Benchmark(b *testing.B) {
	for i := 0; i < b.N; i++ {
		data := []byte("Сжатие_Хаффмана")
		h := &HuffmanService{}
		compressedData, _ := h.Compress(data)
		h.Decompress(compressedData)
	}
}
